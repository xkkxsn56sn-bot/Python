...
def pypdf_available() -> bool:
    return PdfReader is not None and PdfWriter is not None and UAP is not None
...
def make_titled_markdown(src: Path) -> Path:
    title = html.escape(prettify_title(src.stem))
    ...
    tmp = tempfile.NamedTemporaryFile(delete=False, suffix=src.suffix, mode="w", encoding="utf-8")
    tmp.write(titled)
    tmp.close()
...
def build_permissions(allow_print: bool, allow_copy: bool, allow_modify: bool):
    permissions = UAP(0)
    if allow_print:
        for name in ("PRINT", "PRINT_TO_REPRESENTATION"):
            if hasattr(UAP, name):
                permissions |= getattr(UAP, name)
    if allow_copy:
        for name in ("EXTRACT", "EXTRACT_TEXT_AND_GRAPHICS"):
            if hasattr(UAP, name):
                permissions |= getattr(UAP, name)
    if allow_modify:
        for name in ("MODIFY", "ADD_OR_MODIFY", "FILL_FORM_FIELDS", "ASSEMBLE_DOC"):
            if hasattr(UAP, name):
                permissions |= getattr(UAP, name)
    return permissions
...
def protect_pdf(
    ...
    if not owner_password:
        raise ValueError("An owner password is required when PDF protection is enabled")
    ...
    permissions = build_permissions(allow_print, allow_copy, allow_modify)
    writer.encrypt(
    ...
    tmp_path = pdf_path.with_suffix(".protected.pdf")
...
def convert_all(
    ...
    if protect_pdfs and not pypdf_available():
        raise RuntimeError("PDF protection requested, but pypdf is not installed. Run: pip install pypdf")
    if protect_pdfs and not owner_password:
        raise RuntimeError("PDF protection requested, but no owner password was provided")
    ...
    for index, src in enumerate(md_files, start=1):
        ...
        tmp_src = None
        try:
            tmp_src = make_titled_markdown(src)
            cmd = build_command(
                src=tmp_src,
                dst=dst,
        ...
        if run.returncode == 0:
            ...
            if protect_pdfs:
                try:
                    if progress_callback:
                        progress_callback(index - 1, total, f"Protecting {src.name}")
                    protect_pdf(
                        pdf_path=dst,
                        owner_password=owner_password,
                        user_password=user_password,
            ...
            if progress_callback:
                label = "Converted" if status == "converted" else "Failed"
                suffix = " (protected)" if protected else ""
                progress_callback(index, total, f"{label} {src.name}{suffix}")
        else:
            results.append(
                ConversionResult(
                    source=str(src),
    ...
    report = {
        ...
        "title_source": "filename_without_extension",
        "title_style": "bold_centered",
        "protection": {
            "enabled": protect_pdfs,
            "allow_print": allow_print,
            "allow_copy": allow_copy,
            "allow_modify": allow_modify,
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Batch-convert Markdown files to PDF using Pandoc + xelatex with recursive scan, JSON reporting, optional GUI, automatic filename title, and optional PDF protection."
    )
    ...
    p.add_argument("--protect-pdf", action="store_true", help="Apply PDF protection after conversion")
    p.add_argument("--owner-password", default="", help="Owner password used to lock PDF permissions")
    p.add_argument("--user-password", default="", help="Optional open password for the PDF")
    p.add_argument("--allow-print", action="store_true", help="Allow printing protected PDFs")
    p.add_argument("--allow-copy", action="store_true", help="Allow copying/extracting text from protected PDFs")
    p.add_argument("--allow-modify", action="store_true", help="Allow modifying protected PDFs")
...
class App:
    def __init__(self, root):
        ...
        self._build_ui()
        self._toggle_protection_fields()
    ...
    def _build_ui(self):
        ...
        row += 1
        sec = ttk.LabelFrame(frm, text="PDF protection", padding=12)
        sec.grid(row=row, column=0, columnspan=3, sticky="ew", pady=(4, 8))
        ttk.Checkbutton(
            sec,
            text="Protect generated PDFs",
            variable=self.protect_pdf_var,
            command=self._toggle_protection_fields,
        ...
        ttk.Button(deps, text="Check dependencies", command=self._check_dependencies).grid(row=0, column=0, sticky="w")
    ...
    def _toggle_protection_fields(self):
        state = "normal" if self.protect_pdf_var.get() else "disabled"
        widgets = [
    ...
    def _check_dependencies(self):
        ...
        pdf_ok = pypdf_available()
        status = (
            f"pandoc: {'OK' if pandoc_ok else 'missing'} | "
            f"{engine}: {'OK' if engine_ok else 'missing'} | "
            f"pypdf: {'OK' if pdf_ok else 'missing'}"
        )
    ...
    def _start_conversion(self):
        ...
        if not input_dir or not output_dir:
            ...
            return
        if self.protect_pdf_var.get() and not self.owner_password.get().strip():
            messagebox.showerror("Missing owner password", "Please enter an owner password for PDF protection.")
            return
        ...
        def worker():
            try:
                report = convert_all(
                    ...
                    protect_pdfs=self.protect_pdf_var.get(),
                    ...
                    allow_modify=self.allow_modify.get(),
